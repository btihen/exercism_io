...
defmodule RobotSimulatorTest do
  ...
  test "simple L actions" do
    ...
    robot2 = RobotSimulator.create(:east, {1, 1}) |> RobotSimulator.simulate("R")
    assert RobotSimulator.direction(robot2) == :south
    assert RobotSimulator.position(robot2) == {1,1}

    robot3 = RobotSimulator.create(:south, {1, 2}) |> RobotSimulator.simulate("A")
    assert RobotSimulator.direction(robot3) == :south
    assert RobotSimulator.position(robot3) == {1,1}
  end

  test "simple invalid action" do
    assert RobotSimulator.create() |> RobotSimulator.simulate("U") ==
             {:error, "invalid instruction"}

  end

  test "simple compound actions" do
    robot1 = RobotSimulator.create(:south, {1, 2}) |> RobotSimulator.simulate("AL")
    assert RobotSimulator.direction(robot1) == :east
    assert RobotSimulator.position(robot1) == {1,1}

    robot2 = RobotSimulator.create(:south, {1, 2}) |> RobotSimulator.simulate("ALA")
    assert RobotSimulator.direction(robot2) == :east
    assert RobotSimulator.position(robot2) == {2,1}
  end
