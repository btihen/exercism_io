defmodule RobotSimulator do
  ...
  defp do_action(robot, "L") do
    cond do
      :north == robot[:direction] -> %{direction: :west,  position: robot[:position]}
      :east  == robot[:direction] -> %{direction: :north, position: robot[:position]}
      :south == robot[:direction] -> %{direction: :east,  position: robot[:position]}
      :west  == robot[:direction] -> %{direction: :south, position: robot[:position]}
      true                        -> {:error, "invalid direction"}
    end
  end
  defp do_action(robot, "R") do
    cond do
      :north == robot[:direction] -> %{direction: :east,  position: robot[:position]}
      :east  == robot[:direction] -> %{direction: :south, position: robot[:position]}
      :south == robot[:direction] -> %{direction: :west,  position: robot[:position]}
      :west  == robot[:direction] -> %{direction: :north, position: robot[:position]}
      true                        -> {:error, "invalid direction"}
    end
  end
